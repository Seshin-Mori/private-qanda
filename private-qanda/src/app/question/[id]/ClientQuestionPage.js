...
export default function ClientQuestionPage({ id, question: initialQuestion }) {
  const router = useRouter();
  const [question, setQuestion] = useState({
    ...initialQuestion,
    createdAt: new Date(initialQuestion.createdAt), // ミリ秒からDateオブジェクトに変換
  });
        ...
        <div className='space-y-4'>
          {sortedAnswers.map((answer) => (
            <AnswerCard
              ...
              canSetBestAnswer={
                user && question.userId === user.uid && !question.bestAnswerId
              }
            />
          ))}
        </div>
